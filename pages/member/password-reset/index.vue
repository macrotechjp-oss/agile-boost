    ...
    <v-carousel height="100%" cycle vertical vertical-delimiters="true">
      ...
      <v-overlay z-index="5">
            ...
            <v-container>
              <v-col class="mx-auto px-5 text-center black--text">
                ...
                <template v-else>
                  ...
                  <v-row justify="center" class="mb-2">
                    <v-subheader>パスワード再設定用のメールが送信されました。</v-subheader>
        ...
        >
          <v-row align="center">
            <v-col class="grow">
              パスワード再設定メール送信処理に失敗しました。再度、お試しください。
            </v-col>
          </v-row>
        </v-alert>
      </v-overlay>
...
export default {
  ...
  methods: {
    ...
    async passwordReset () {
      ...
      let res
      try {
        res = await this.$axios.post('/api/v1/auth/send-password-reset', { userEmail: this.email })
        // パスワードリセットメール送信処理成功時
